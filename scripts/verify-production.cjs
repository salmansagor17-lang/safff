...
async function main() {
  ...
  assert.equal(payload.categories.length, 11); assert.equal(questions.length, 2100);
  assert.equal(new Set(questions.map(q => q.id)).size, 2100);
  ...
  const session = ctx.window.SessionQuestions.build(payload.categories, payload.categories.map(c => c.id));
  assert.equal(session.flatMap(c => c.questions).length, 165);
  for (const category of session) for (const points of [100,200,300,400,500]) assert.equal(category.questions.filter(q => q.points === points).length, 3);
  ...
  assert.equal(local.length, 2100);
  ...
  console.log(JSON.stringify({ status: 'PASS', base, categories: payload.categories.map(c => ({ name: c.category, count: c.questions.length })),
    total: questions.length, images: 550, symbols: 50, sessionQuestions: 165, mediaUrlsChecked: images.length, localBank: 'matched', adminFiles: '404' }, null, 2));
}
